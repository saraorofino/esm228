...
Stratified sampling with sample weights is one of the simplest ways to confront sampling bias.  
...
We can assume that the respondents from the main street are random and the respondents from the side streets are random. By splitting the population into two main groups we can randomly sample within these gruops and then find weighted averages for the population based on how much of the population falls within each strata.     
...
Estimating how to do this with sampling  

```{r strata-sam-dist, cache=TRUE}
sims <- 1000 #simulations
sam.n <- 250 #attempted sample size
store <- rep(NA, sims) #df to store the simulations so we can simulate the distribution

for (i in 1:sims){
  index <- sample(1:500,sam.n) #draw a sample
  pop <- reporting(pop) # actual population
  main <- mean(pop[index,] %>%
               filter(R==1 & main==1) %>% # Find main road participants who are likely to answer the door
               pull(satisfied)) # extract the proportion satisfied
  side <- mean(pop[index,] %>%
               filter(R==1 & main==0) %>% # Find side road participants who are likely to answer the door
               pull(satisfied)) # extract the proportion satisfied 
  store[i] <- main * 0.5 + side * 0.5 # weighting these 50-50 because our population is half on main road, half not
...
2. The distribution of the population into strata is known
   - Here we assume that half of the population lives on main roads and half live on side roads. This breakdown needs to be known  
...
```
Why are there larger tails in the side street if we sample at the same rate?  
 - The main street residents respond at a higher rate so we effectively get more samples from the main road participants and are therefore closer to the mean then when we sample from the side street population, where we have less responses and thereofore a higher deviation from the mean.  
...
```
We could sample at a higher rate in the side street if we are more interested in precise estimates of dissatisfaction, which we think is more likley with residents on side streets.  
...
for (i in 1:sims){
  sam <- c(sample(main.index,75),
           sample(side.index,175)) #drawn sample but this time it is different for the two groups
  pop <- reporting(pop)
  main <- mean(pop[sam,] %>%
               filter(R==1 & main==1) %>% # participants on the main road who answer the door
               pull(satisfied)) # extract the proportion satisfied 
  side <- mean(pop[sam,] %>%
               filter(R==1 & main==0) %>% # participants on side road who answer the door
               pull(satisfied)) # extract the proportion satisfied 
  store[i] <- main * 0.5 + side * 0.5 # main and side street residents are 50-50
...
```
Proportionate sampling - we knock on the same number of doors on main roads and side roads. But since the main road participants are more likely to answer the door there is a larger proportion of main street residents in the sample. We can make a more precise claim about these residents.  

Disproportionate sampling - Because we know main street residents answer their doors at higher rates, we decide to only knock on 75 doors on the main road and 175 doors on the side roads. This gives us a lower proportion of main street residents and makes that estimate a little less precise, but it makes the proportion of side street residents in the sample larger and gains precision. Since we are interested in policy based on the responses from the side street subgroup we want to enhance precision with our sampling in this group.  
