...
## DeclareDesign()
This is talking about the possible reporting or samplinig bias  
 - We don't suspect there will be much reporting bias because the question doesn't involve any illegal or socially undesirable behaviors  
 - We assume that any given person you approach would have an 80% chance of responding to the survey  
```{r report-samp}
...
```
Sampling procedure: stratify by port because the number of captains at each port are pretty different  

## DeclareDesign()
You can use this to calculate a strata weighted mean  
```{r declare-estimator}
...
## DeclareDesign()
Here the sd_estimate is the margin of error that we are interest in  
